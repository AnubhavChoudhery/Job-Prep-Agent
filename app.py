...
# import time  # <-- commented out timing
...
def process_jobs(resume_file, role: str, location: str, num_jobs: int, progress=gr.Progress()):
    try:
        ...
        # elapsed = time.time() - t0  # <-- commented out
    ...
    with gr.Blocks(title="Job Search Assistant", theme=gr.themes.Soft()) as app:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                excel_download = gr.File(label="Download Excel Report", visible=False)
                documents_download = gr.File(label=" Download Interview Documents (ZIP)", visible=False)
        process_btn.click(
...
def main():
    ...
    app.launch(server_name="0.0.0.0", server_port=8000, share=True, show_api=False)
